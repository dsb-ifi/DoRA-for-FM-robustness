...
def get_global_size() -> int:
    ...
    """
    #gs = dist.get_world_size() if is_enabled() else 1
    #print("Getting global size, which is", gs, flush=True)
...
def get_local_rank() -> int:
    ...
    if not is_enabled():
        return 0
    #print("LOCAL rank, ws", _LOCAL_RANK, _LOCAL_WORLD_SIZE)
...
class _TorchDistributedEnvironment:
    ...
    def _set_from_preset_env(self):
        ...
        assert self.local_rank < self.local_world_size
        print(f"Setting distributed parameters from preset. Local rank {self.local_rank}, World size {self.world_size}")

    # Single node and GPU job (i.e. local script run)
    def _set_from_local(self):
        # logger.info("Initialization from local")
        print("Set from local: Using only one GPU: 3")
        self.master_addr = "127.0.0.1"
...
def enable(*, set_cuda_current_device: bool = True, overwrite: bool = False, allow_nccl_timeout: bool = False):
    ...
    if set_cuda_current_device:
        ...
        print("Current device", torch_env.local_rank)
    ...
    #print("Env. local rank and ws", torch_env.local_rank, _LOCAL_WORLD_SIZE)

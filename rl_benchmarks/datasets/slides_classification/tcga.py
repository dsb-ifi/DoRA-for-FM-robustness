...
def load_histo(
    ...
    """
    # Get paths
    slides_root_dir = Path(TCGA_PATHS["SLIDES"]) / cohort.replace("_", "-") / "Diagnostic_slides"
    coords_root_dir = Path(TCGA_PATHS["COORDS"](tile_size))  / cohort.split("_")[-1] #/ cohort
    ...
    print("Feature dir", features_root_dir, flush=True)
...
def load_label(
    ...
    def _load_clin(
        ...
        if "my_survival" in df_clin.keys():
    ...
    # starting with survival
        ...
        df_labels = df_labels.drop(df_labels[df_labels["STAGE"]=='[Discrepancy]'].index)
...
def load_tcga(
    ...
    #print("in load_tcga", dataset.shape, dataset.keys())
    if label is not None:
        ...
        #print("Labels", df_labels.shape, df_labels.head(1))

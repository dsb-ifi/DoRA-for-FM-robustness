...
"""S36 dataset loading for a given cohort and label."""
...
from ...constants import S36_PATHS, S36_TASKS
...
def load_histo(
    ...
    """
    # Get paths
    slides_root_dir = Path(S36_PATHS["SLIDES"]) #/ cohort.replace("_", "-")
    coords_root_dir = Path(S36_PATHS["COORDS"](tile_size)) / cohort.split("_")[1]
    ...
    # Drop the slides that dont have coordinates (S36 slides is not divided between LUAD/LUSC)
...
def load_label(
    ...
    def _load_clin(
        ...
        df_clin["DSS"] = df_clin.apply(
            lambda row: row["OS.time"] if row["DSS"] == 1 else -row["OS.time"],
            axis=1,
        )
    ...
    # starting with survival
        ...
        labels_path = Path(S36_PATHS["LABELS"]["SURVIVAL"](cohort))
        ...
        convert_labels = {'IA3':'I', 'IA2':'I', 'IA1': 'I', 'III':'III', 'IIIA':'III', 'IIB':'II', 'IIIC':'III', 'IA':'I', 'II':'II', 'IVA':'IV', 'I':'I', 'IB':'I', 'IIIB':'III', 'IV':'IV', 'IIA':'II', 'IVA':'IV', 'IVB':'IV', 'IIIC':'III'}
...
def load_s36(
    ...
    """
    assert (
        label in S36_TASKS[cohort]
    ), f"Choose a label in {S36_TASKS[cohort]} for cohort {cohort}. Provided {label}."
    dataset = load_histo(
        cohort=cohort,

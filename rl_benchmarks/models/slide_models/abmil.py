...
class ABMIL(nn.Module):
    ...
    def __init__(
        ...
        mlp_hidden: Optional[List[int]] = None,
        mlp_dropout: Optional[List[float]] = None,
        mlp_initial_dropout: Optional[float] = 0.0,
        mlp_activation: Optional[torch.nn.Module] = torch.nn.Sigmoid(),
        bias: bool = True,
        metadata_cols: int = 3,
    ) -> None:
        super(ABMIL, self).__init__()
        print("Using ABMIL model")
        ...
        # self.proj = nn.Linear(d_model_attention, d_model_attention)
        # self.drop = nn.Dropout(mlp_initial_dropout)
        ...
        self.mlp = MLP(
            ...
            hidden=mlp_hidden,
            dropout=mlp_dropout,
            initial_dropout=mlp_initial_dropout,
    ...
    def forward(
        ...
        #tiles_emb = self.drop(self.proj(tiles_emb))

...
class MLP(torch.nn.Sequential):
    ...
    def __init__(
        ...
        hidden: Optional[List[int]] = None,
        dropout: Optional[List[float]] = None,
        initial_dropout: Optional[float] = 0.0,
        activation: Optional[torch.nn.Module] = torch.nn.Sigmoid(),
        bias: bool = True,
    ):
        ...
        if hidden is not None:
            for i, h in enumerate(hidden):
                ...
                if i==0 and initial_dropout > 0:
                    seq.append(torch.nn.Dropout(initial_dropout))
                
                seq.append(torch.nn.Linear(d_model, h, bias=bias))
                d_model = h

...
def compute_binary_accuracy(labels: np.array, logits: np.array) -> float:
    ...
    logits = logits[labels<2]
    labels = labels[labels<2] 

    preds = np.where(logits > 0, 1, 0)
    return accuracy_score(labels, preds)

def compute_binary_accuracy_balanced(labels: np.array, logits: np.array) -> float:
    """Binary accuracy.
    Parameters
    ----------
    labels: np.array
...
def compute_multiclass_accuracy_balanced(labels: np.array, logits: np.array) -> float:
    """Multi-class accuracy.
    Parameters
    ----------
    labels: np.array
...
def compute_binary_auc(labels: np.array, logits: np.array) -> float:
    ...
    try:
        ...
        print("ValueError in roc_auc_score; binary auc.", e)
        # import IPython
        # IPython.embed()
        # import sys
        # sys.exit()

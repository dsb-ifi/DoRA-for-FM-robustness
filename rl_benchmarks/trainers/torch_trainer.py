...
class TorchTrainer(BaseTrainer):
    ...
    def train(
        ...
        # Dataloaders.
        if balance_loader:
            print("Balancing training dataloader wrt stratification")
            
            if "ConcatDataset" in str(type(train_set)): # If training dataset is concatenated of 2 datasets
                num_samples = 0
                strat_1 = 0
                sample_weights = []
                for d in train_set.datasets:
                    num_samples += len(d)
                    strat_1 += d.stratified.sum()
                strat_counts = [num_samples-strat_1, strat_1]
                for d in train_set.datasets:
                    sample_weights += [1/strat_counts[i] for i in d.stratified]
                train_sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)
            elif "Subset" in str(type(train_set)):
                num_samples = len(train_set.indices)
                strat_1 = train_set.dataset.stratified[train_set.indices].sum()
                strat_counts = [num_samples-strat_1, strat_1]
                sample_weights = [1/strat_counts[i] for i in train_set.dataset.stratified[train_set.indices]]
                train_sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)
            else: # Not ConcatDataset or Subset type
                num_samples = len(train_set)
                strat_1 = train_set.stratified.sum()
                strat_counts = [num_samples-strat_1, strat_1]
                sample_weights = [1/strat_counts[i] for i in train_set.stratified]
                train_sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)
                #print("Sample weights:", len(sample_weights), sample_weights)
        ...
        # Explore how mini-batches are constructed (distribution of survival within a mini-batch)
        # import IPython
        # IPython.embed()
        # sys.exit()

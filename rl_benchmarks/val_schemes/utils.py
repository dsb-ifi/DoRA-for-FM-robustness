...
def split_cv(
    ...
    shuffle: bool = True,
    random_state: int = 42,
    test_dataset: Optional[Union[Dataset, Subset, SlideClassificationDataset]] = None,
    ...
    if stratified:
        if isinstance(dataset, ConcatDataset):
            ...
            for d in dataset.datasets:
                y = np.append(y, d.stratified)
                patient_ids = np.append(patient_ids, d.patient_id)
                center_ids = np.append(center_ids, d.center_id)
